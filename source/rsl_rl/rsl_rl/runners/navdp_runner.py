...
import rsl_rl
from rsl_rl.algorithms import PPO, Distillation, DiffusionPPO, DiffusionDistillation
...
class NavdpRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] in ["PPO", "DiffusionPPO"]:
            self.training_type = "rl"
        elif self.alg_cfg["class_name"] in ["Distillation", "DiffusionDistillation"]:
        ...
        policy: NavdpPolicy = policy_class(env_num=self.env.num_envs,
            **self.policy_cfg
        ).to(self.device)
        ...
        self.alg: DiffusionPPO | DiffusionDistillation = alg_class(
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            # -- episode info
            log_string +=f"""{'Cur reward:':>{pad}} {torch.mean(locs['cur_reward_sum']):.2f}\n"""
    ...
    def save(self, path: str, infos=None):
        ...
        torch.save(self.alg.policy.state_dict(), path.replace(".pt", "_navdp.pt"))
