...
def adjust_usd_scale(prim_path="/World/Scene/terrain", scale=1.0, rotation=None):
    import omni
    from pxr import UsdGeom, Usd, Sdf, Gf
    stage = omni.usd.get_context().get_stage()
    scene_prim = stage.GetPrimAtPath(prim_path)
    if scene_prim.IsValid():
        print(f"Directly setting scale and rotation for prim: <{scene_prim.GetPath()}>")
        ...
        # 2. Get or create the orient attribute and set its value if rotation is provided.
        if rotation is not None:
            # rotation: [w, x, y, z] (Gf.Quatd)
            orient_attr = scene_prim.GetAttribute("xformOp:orient")
            if not orient_attr:
                orient_attr = scene_prim.CreateAttribute("xformOp:orient", Sdf.ValueTypeNames.Quatd, False)
            # If rotation is a list/tuple of 4 numbers: [w, x, y, z]
            if isinstance(rotation, (list, tuple)) and len(rotation) == 4:
                ...
                print("Warning: rotation should be a list or tuple of 4 elements [w, x, y, z].")

        # 3. Ensure 'xformOp:scale' and 'xformOp:orient' are in the transformation order.
        order_attr = scene_prim.GetAttribute("xformOpOrder")
        required_ops = ["xformOp:translate", "xformOp:orient", "xformOp:scale"]
        if not order_attr.HasValue():
            scene_prim.CreateAttribute("xformOpOrder", Sdf.ValueTypeNames.TokenArray, False).Set(required_ops)
        else:
        ...
        print(f"Successfully set scale and rotation for prim <{scene_prim.GetPath()}>")
    else:
        print("Warning: Could not find prim at /World/Scene to apply scale and rotation.")
