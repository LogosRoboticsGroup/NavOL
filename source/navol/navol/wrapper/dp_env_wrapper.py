...
class RslRlDPEnvWrapper(VecEnv):
    ...
    def get_observations(self) -> tuple[torch.Tensor, dict]:
        ...
        return self.process_obs(obs_dict), {"observations": obs_dict}

    def process_obs(self, obs_dict):
        obs = obs_dict["policy"].copy()
        for k in obs_dict["policy"].keys():
            if k != "rgbd_token" and "goal_pose" not in k:
                obs.pop(k)
        obs_tensor = torch.cat([v.reshape(self.num_envs, -1) for v in obs.values()], dim=-1)
        return obs_tensor
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        obs = self.process_obs(obs_dict)
        extras["observations"] = obs_dict
        extras["actions"] = {
            "joint_policy": self.env.unwrapped.action_manager.get_term("joint_combined").policy_actions,
            "joint_command": self.env.unwrapped.action_manager.get_term("joint_combined").command_actions,
            "critic_values": self.env.unwrapped.action_manager.get_term("joint_combined").critic_values,
            "is_valids": self.env.unwrapped.action_manager.get_term("joint_combined").is_valids,
        }
