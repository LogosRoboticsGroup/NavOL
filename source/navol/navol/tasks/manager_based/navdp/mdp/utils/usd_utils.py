...
def add_point_light(
    ...
    # point_light = UsdLux.DomeLight(light_prim)
    point_light.CreateIntensityAttr(intensity)
    point_light.CreateColorAttr(Gf.Vec3f(*color))
    # point_light.CreateGuideRadiusAttr(1)
    point_light.CreateRadiusAttr(radius)
    
    xform_prim = XFormPrim(prim_path)
    xform_prim.set_world_poses(positions=torch.as_tensor(position, dtype=torch.float32, device='cuda').unsqueeze(0))
    return point_light