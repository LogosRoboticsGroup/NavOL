...
if TYPE_CHECKING:
    ...
    from .commands_cfg import TaskCommandCfg, UniformVelocityCommandStandingCfg
...
class TaskCommand(CommandTerm):
    ...
    def __init__(self, cfg: TaskCommandCfg, env: ManagerBasedEnv):
        ...
        self.task_command[:, 0] = 1.0  # default to point
        # -- task probability [0.5, 0.0, 0.5, 0.0]
        self.task_prob = cfg.task_prob

    def __str__(self) -> str:
        """Return a string representation of the command generator."""
        msg = "UniformVelocityCommand:\n"
        msg += f"\tCommand dimension: {tuple(self.command.shape[1:])}\n"
        msg += f"\tTask_prob: {self.cfg.task_prob}\n"
        return msg
    ...
    @property
    def command(self) -> torch.Tensor:
        """The desired goal task. Shape is (num_envs, 4)."""
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        rand_task = torch.multinomial(torch.tensor(self.task_prob, device=self.device), len(env_ids), replacement=True)
        self.task_command[env_ids] = torch.nn.functional.one_hot(rand_task, num_classes=4).float()
