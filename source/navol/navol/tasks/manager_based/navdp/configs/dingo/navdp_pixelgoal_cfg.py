...
class PixelNavObservationsCfg:
    ...
    class GoalPixelCfg(ObsGroup):
        pixel_measurement = ObsTerm(
            func = mdp.pixel_projection_data,
...
class PixelNavEventCfg:
    ...
    reset_pose = EventTerm(func=mdp.pixelnav_reset,
                           mode='reset',
...
class CommandsCfg:
    ...
    task_command = mdp.TaskCommandCfg(
        ...
        task_prob=[0.0, 0.0, 0.0, 1.0], # pixelgoal
    )

@configclass
class DingoPixelNavCfg(ManagerBasedRLEnvCfg):
    scene: InteractiveSceneCfg = PixelNavSceneCfg(env_spacing=0.0)
