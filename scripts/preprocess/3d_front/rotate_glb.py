...
def process_scenes(args):
    ...
    print(f"Starting conversion: {args.input_path}")
    clear_scene()
    ...
    merged.matrix_world = rot_x @ merged.matrix_world
    bpy.ops.object.transform_apply(location=True, rotation=True, scale=False)
    print("Rotation reverted by -90 degrees")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_path", type=str, help="Input GLB file path")
    parser.add_argument("--output_path", type=str, help="Output GLB file path")
    argv = sys.argv
    if "--" in argv:
