...
def process_scenes(args):
    ...
    target_dir = os.path.join(args.scene_path, "scenes")
    target_dir_navmesh = os.path.join(args.scene_path, "navmesh_scenes")
    os.makedirs(target_dir, exist_ok=True)
    os.makedirs(target_dir_navmesh, exist_ok=True)
    
    processed_count = 0
    failed_count = 0
    
    for idx, (scene_name, scene_data) in enumerate(scene_dict.items()):
        print(f"\nProcessing scene {idx+1}/{len(scene_dict)}: {scene_name}")
        scene_path = scene_data['path']
        if not args.benchmark:
            target_path = os.path.join(target_dir, scene_name + ".glb")
            target_path_navmesh = os.path.join(target_dir_navmesh, scene_name + ".glb")
        else:
            target_dir = os.path.join(args.scene_path, scene_name)
        ...
        print(f"Navmesh Output: {target_path_navmesh}")
        print(f"{'='*60}")
        if os.path.exists(target_path) and os.path.exists(target_path_navmesh):
            success = True
        else:
            success = convert(scene_path, target_path, target_path_navmesh, args.target_height)
        ...
        if success:
            ...
            failed_count += 1
            print(f"✗ Failed to process: {scene_path}")
    
    print(f"\n{'='*60}")
    print(f"Batch processing completed!")
    print(f"Successfully processed: {processed_count} files")
    print(f"Failed: {failed_count} files")
    print(f"Total: {len(scene_dict)} files")
    print(f"{'='*60}")
