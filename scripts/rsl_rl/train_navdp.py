...
parser.add_argument("--rand_camera", action=argparse.BooleanOptionalAction, default=None,
                    help="Override canonical camera randomization (use --no-rand_camera to disable).")
...
from navol.wrapper import RslRlOnPolicyRunnerCfg, RslRlDPEnvWrapper, CustomVideoWrapper
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.rand_camera is not None:
        env_cfg.events.reset_pose.params["rand_camera"] = args_cli.rand_camera
    ...
    is_main_process = (args_cli.distributed and app_launcher.global_rank == 0) or not args_cli.distributed
    ...
    if args_cli.video and is_main_process:
        video_kwargs = {
        ...
        env = CustomVideoWrapper(env, **video_kwargs)
    ...
    runner = NavdpRunner(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
