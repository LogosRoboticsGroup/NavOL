...
parser.add_argument(
    "--usd_path", type=str, default=None)
parser.add_argument(
    "--init_path", type=str, default=None)
parser.add_argument(
...
parser.add_argument(
    "--light_enabled", action="store_true", default=False)
parser.add_argument(
    "--wo_critic", action="store_true", default=False)
parser.add_argument(
    "--max_linear_speed", type=float, default=0.5)
parser.add_argument(
    "--factor_scale_vel", type=float, default=2.0)
parser.add_argument(
    "--factor_scale_ang", type=float, default=4.0)
parser.add_argument(
    "--vel_scale", type=float, default=2.0)
parser.add_argument(
    "--angular_scale", type=float, default=1.0)
...
def main():
    ...
    if args_cli.scene_dir is not None:
        if 'internscenes_commercial' in args_cli.scene_dir:
            prefix = 'internscenes_commercial'
        elif 'internscenes_home' in args_cli.scene_dir:
            prefix = 'internscenes_home'
        elif 'cluttered_easy' in args_cli.scene_dir:
            prefix = 'cluttered_easy'
        elif 'cluttered_hard' in args_cli.scene_dir:
            prefix = 'cluttered_hard'
        elif 'benchmark_in_domain' in args_cli.scene_dir:
            prefix = 'benchmark_in_domain'
        elif 'benchmark_out_domain' in args_cli.scene_dir:
            prefix = 'benchmark_out_domain'
        else:
            prefix = 'others'
    
        scene_path = os.path.join(args_cli.scene_dir,sorted(os.listdir(args_cli.scene_dir))[args_cli.scene_index])
        usd_path, init_path = find_usd_path(scene_path, task='')
        if args_cli.extra is not None:
            args_cli.save_path = os.path.join(args_cli.save_path, prefix + '_' + os.path.basename(scene_path)+'_'+args_cli.extra)
        else:
            args_cli.save_path = os.path.join(args_cli.save_path, prefix + '_' + os.path.basename(scene_path))
    else:
        usd_path = args_cli.usd_path
        init_path = args_cli.init_path
        if args_cli.extra is not None:
            args_cli.save_path = os.path.join(os.path.dirname(args_cli.save_path), prefix + '_' + os.path.basename(args_cli.save_path) + '_' + args_cli.extra)
    print(f"[INFO] Loading Scene from USD: {usd_path}")
    print(f"[INFO] Loading Trajectory from NPY: {init_path}")
    print(f"[INFO] Saving Results to: {args_cli.save_path}")
    ...
    env_cfg.events.reset_pose.params["light_enabled"] = args_cli.light_enabled
    env_cfg.actions.joint_combined.factor_scale_vel = args_cli.factor_scale_vel
    env_cfg.actions.joint_combined.factor_scale_ang = args_cli.factor_scale_ang
    env_cfg.actions.joint_combined.vel_scale = args_cli.vel_scale
    env_cfg.actions.joint_combined.angular_scale = args_cli.angular_scale
    env_cfg.actions.joint_combined.max_linear_speed = args_cli.max_linear_speed
    env_cfg.actions.joint_combined.use_mpc = args_cli.use_mpc
    env_cfg.actions.joint_combined.use_critic = not args_cli.wo_critic
    if 'n1_eval_scenes' in usd_path:
        env_cfg.actions.joint_combined.scene_scale = 1.0
    else:
    ...
    evaluation_metrics = []
    episode_idx = np.arange(env.num_envs)
    ...
    if args_cli.save:
        os.makedirs(os.path.join(args_cli.save_path, "videos", ), exist_ok=True)
        fps_writer = [imageio.get_writer(os.path.join(args_cli.save_path, "videos", f"traj_{i:03d}.mp4"), fps=10) for i in range(args_cli.num_envs)]
        fps_depth_writer = [imageio.get_writer(os.path.join(args_cli.save_path, "videos", f"traj_{i:03d}_depth.mp4"), fps=10) for i in range(args_cli.num_envs)]

    # simulate environment
    while simulation_app.is_running():
        last_infos = infos.copy()
        with torch.inference_mode():
            # goals = last_infos['observations']["policy"]['goal_pose'].cpu().numpy()[:,0:2]
            # images = (last_infos['observations']["policy"]['rgb'].permute(0,2,3,1).cpu().numpy()*255).astype(np.uint8)
            # depths = last_infos['observations']["policy"]['depth'].permute(0,2,3,1).cpu().numpy()
            # current_trajectory_camera, all_trajectories_camera, current_all_values, rgbd_embed, image, input_image, input_depth = pointgoal_step(goals.copy(), images.copy(), depths.copy(),port=8881)
            # # actions = current_trajectory_camera.reshape(env.num_envs, -1)
            # # actions = (current_trajectory_camera - np.concatenate([np.zeros_like(current_trajectory_camera[:,0:1]), current_trajectory_camera[:, :-1]],axis=1)) * 4
            # # actions = torch.from_numpy(actions).to(env.device).float().reshape(env.num_envs, -1)
            # #  env.unwrapped.observation_manager._group_obs_term_cfgs['policy'][4].func.input_depth[0].sum()
            # #  env.unwrapped.observation_manager._group_obs_term_cfgs['policy'][4].func.input_image[0].sum()
            # if np.abs(input_depth[0]-env.unwrapped.observation_manager._group_obs_term_cfgs['policy'][4].func.input_depth[0]).sum() != 0 or np.abs(input_image[0]-env.unwrapped.observation_manager._group_obs_term_cfgs['policy'][4].func.input_image[0]).sum() != 0:
            #     print("Different input detected!")
            # obs[..., :-3] = torch.tensor(rgbd_embed[:1], device='cuda', dtype=torch.float32).reshape(1, -1)
            actions = policy(obs)
            ...
            # print("veloity:", infos['observations']["policy"]['base_lin_vel'].cpu().numpy()[:,0])
        ...
        for i in range(env.num_envs):
            ...
            if dones[i] == True and episode_idx[i] < args_cli.num_episodes:
                if args_cli.task != "nogoal":
                    ...
                    write_metrics(evaluation_metrics, os.path.join(args_cli.save_path, f"metric_{args_cli.task}.csv"))
                else:
                    raise NotImplementedError
                if args_cli.save:
                    fps_writer[i].close()
                    fps_depth_writer[i].close()
                    fps_writer[i] = imageio.get_writer(os.path.join(args_cli.save_path, "videos", f"traj_{episode_num:03d}.mp4"), fps=10)
                    fps_depth_writer[i] = imageio.get_writer(os.path.join(args_cli.save_path, "videos", f"traj_{episode_num:03d}_depth.mp4"), fps=10)
            ...
            if dones[i] == True:
                episode_idx[i] = episode_num
        
        if episode_idx.min() >= args_cli.num_episodes:
            break
